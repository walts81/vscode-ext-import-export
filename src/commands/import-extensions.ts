...
const readFile = (path: string): Promise<{ plugins: ExtensionInformation[] }> => {
...
export default async (context: ExtensionContext) => {
  ...
  const json = await readFile(uris[0].path);
  try {
    await pluginService.installExtensions(env.osType, json.plugins, vscodeHelpers.showInformationMessage);
    const answer = await vscodeHelpers.showInformationMessage('Finished installing extensions. Please reload VS Code.', 'Reload');
    if (answer === 'Reload') {
